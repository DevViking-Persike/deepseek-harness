...
The Git access seam — a [capability seam](../../.agents/notes/implemented/architecture/2026-08-23-git-capability-seam-and-editor-panel.md) that spans repository discovery, working-tree status, diffs, history, worktrees, base comparison, and index/worktree mutation on one `ctx.git` service, split across packages: Service Definition ([dsh-git](../../packages/git/git), `ctx.git` + the provider registry), Service Provider ([dsh-git-local](../../packages/git/git-local), the local `git` CLI over `ctx.subprocess`), and Consumer ([dsh-tool-git](../../packages/git/tool-git), the `git_status` / `git_diff` / `git_log` reads plus the opt-in mutating group). Git is **one optional capability**, not part of the agent-loop spine, so its vocabulary lives here rather than in [core.md](core.md).

Source: [`packages/git/git/src/types.ts`](../../packages/git/git/src/types.ts)

## Why these operations are one capability

Discovery, status, diffs, history, worktrees, and mutation all resolve one repository through one backend and fail identically when that backend is unusable, so they share one provider-selection policy owner, one abort/error vocabulary, and one "how this harness reaches Git" configuration API. Their request and result types stay separate. Providers register a **backend** (a `GitProvider`), not tools; the model-facing names, schemas, prompt guidance, and presentation live in the single `dsh-tool-git` consumer.

Availability is re-probed per call rather than cached: a repository is initialized, cloned, or deleted during an ordinary session, so a cached answer routes an operation to a backend that no longer applies.

## Changes

`GitChangeKind` is a CLOSED union: consumers `switch` on it to exhaustiveness, and a backend reporting a state outside the set maps it to the closest member rather than widening the union.

`GitFileChange` carries `index` and `worktree` **separately** because Git's two-letter status is genuinely two facts: a file can be staged as added and then modified again. Collapsing them is what makes a UI unable to offer stage and unstage on the same row.
...
A workspace holds more than one repository: a monorepo carries submodules, and a person keeps several projects side by side. Nesting is preserved rather than pruned, with inner repositories marked.
...
A repository holds more than one checkout. The states are what a consumer must separate before offering an action: a locked worktree refuses removal, a prunable one has already lost its directory, and a bare repository has no working tree at all.
...
`behind > 0` is the fact that decides whether a push is safe. `conflicts` is computed in the object database, so asking before a push writes nothing and moves no ref.
...
Parents are what give a graph its lanes: the first parent continues a strand, and each further parent of a merge opens another.
...
`GitDiff` carries whole-file contents rather than a unified patch: the browser draws both sides in full, and a consumer staging a hunk needs the exact bytes.
...
Discarding is the one operation here that can lose uncommitted work, so the seam requires providers to preserve what they replace.
...
A provider registers a backend, never tools. `available()` reports whether it can serve requests right now, and the seam calls it during selection so an unusable backend is skipped rather than chosen and failed.
