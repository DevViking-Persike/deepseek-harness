...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-docker': { kind: 'none', reason: 'Browser-side operator tab; the model\'s own view of the same containers stays with dsh-tool-docker.' },
  'packages/client/ui-editor': { kind: 'none', reason: 'Browser-side operator tab; a file it edits reaches the model only when a later turn reads it through an ordinary tool.' },
  'packages/client/ui-git': { kind: 'none', reason: 'Browser-side operator panel; the model\'s own view of the same repository stays with dsh-tool-git.' },
  'packages/git/git': { kind: 'indirect', reason: 'The service interface delegates all model rendering to dsh-tool-git.' },
  'packages/git/git-local': { kind: 'indirect', reason: 'The CLI backend delegates model rendering to dsh-tool-git.' },
  'packages/client/ui-knowledge': { kind: 'none', reason: 'Browser-side settings section; it reports which skills and documents a session can reach and registers no prompt, schema, or tool of its own.' },
  'packages/host/monaco-assets': { kind: 'none', reason: 'Static HTTP route serving the editor distribution to the browser; it never participates in request assembly.' },
  ...
  'packages/docker/docker': { kind: 'indirect', reason: 'The provider registry delegates model rendering to dsh-tool-docker.' },
  'packages/docker/docker-local': { kind: 'indirect', reason: 'The local CLI backend delegates model rendering to dsh-tool-docker.' },
