 ...
 * llm-cliproxy boot from a test-only cordis.yml through the actual Loader +
 * Include path, external edits of settings.yaml and the credentials document
 * hot-publish through their providers, and the very next request carries the
 * fresh base URL and credential. The same adapter composition without
 * settings or credentials entries keeps entry-config behavior — the
 * documented optional-inject fallback. Disposing the fiber removes both
 * provider routes (HMR safety).
 */
...
const KEY_REF = credentialRef('CLIPROXY_API_KEY')
...
async function loadComposition(
  ...
  root = options.reuseRoot ?? await mkdtemp(join(tmpdir(), 'dsh-llm-cliproxy-'))
  vi.stubEnv('DSH_HOME', root)
  const settingsPath = join(root, 'settings.yaml')
  const credentialsPath = join(root, '.credentials.yaml')
  if (options.withDynamic && fresh) {
    await writeFile(settingsPath, '# personal settings\n')
    await writeFile(credentialsPath, 'CLIPROXY_API_KEY: boot-key\n', { mode: 0o600 })
  }
  ...
  await writeFile(configPath, [
    ...
    ...options.withDynamic
      ...
      : [],
    '- id: llm-cliproxy',
    "  name: '@deepseek-ai/dsh-llm-cliproxy'",
    '  config:',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@deepseek-ai/dsh-llm-cliproxy', LlmCliProxy],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('llm-cliproxy real dynamic composition', () => {
  it('boots from cordis.yml and routes the next request after external settings and credential edits', async () => {
    ...
    await assemble(ctx, { model: 'claude-sonnet-5', messages: [] })
    ...
    await writeFile(settingsPath, `llm-cliproxy:\n  baseURL: ${serverB.url}\n`)
    await vi.waitFor(() => {
      expect((ctx.get('settings')!.get(NS) as { baseURL?: string }).baseURL).toBe(serverB.url)
    }, { timeout: 5000 })
    await writeFile(credentialsPath, 'CLIPROXY_API_KEY: rotated-key\n', { mode: 0o600 })
  ...
  it('keeps a stored key writable and rotatable across a real restart', async () => {
    ...
    await assemble(boot.ctx, { model: 'claude-sonnet-5', messages: [] })
    ...
    await assemble(restarted.ctx, { model: 'claude-sonnet-5', messages: [] })
  ...
  it('boots the same adapter on entry config alone, resolving the reference from the environment', async () => {
    ...
    vi.stubEnv('CLIPROXY_API_KEY', 'entry-key')
  ...
  it('serves both provider routes from one registration and removes them on dispose', async () => {
    vi.stubEnv('CLIPROXY_API_KEY', 'entry-key')
    ...
    const claude = await ctx.llm.listModels('cliproxy-claude')
    const openai = await ctx.llm.listModels('cliproxy-openai')
    expect(claude.map(model => model.id)).toContain('claude-sonnet-5')
    expect(openai.map(model => model.id)).toContain('gpt-5.6-sol')

    // The openai route reaches the same endpoint and credential.
    await assemble(ctx, { provider: 'cliproxy-openai', model: 'gpt-5.6-sol', messages: [] })
    expect(server.requests[0]).toMatchObject({ model: 'gpt-5.6-sol' })

    // HMR safety: disposing the root fiber tears the composition down with
    // it — neither provider route outlives the plugin that registered them.
