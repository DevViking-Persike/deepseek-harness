/**
 * Translate CLIProxyAPI SSE payloads with one stateful harness block per
 ...
 * shapes while ensuring no chunk follows `finish`.
 *
 * @module dsh-llm-cliproxy/translate
 */
 ...
 * counts: cache hits reported inside `prompt_tokens` are subtracted out of
 * `inputTokens`.
 * @param usage - wire usage from the finish chunk or a trailing usage-only chunk.
...
export async function* translate(payloads: AsyncIterable<string>): AsyncGenerator<StreamChunk> {
  ...
  for await (const payload of payloads) {
    ...
    for (const choice of chunk.choices ?? []) {
      ...
      // Reasoning first: thinking upstreams interleave it before text. The
