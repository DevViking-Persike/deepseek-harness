...
The sidebar occupies either outer column: `ctx.layout.toggleSidebarSide()` mirrors the frame, moving the sidebar track between the first and third grid columns and swapping the two panels' borders. Drag deltas invert with the placement, so each handle still widens its own panel when dragged away from the viewport center. Placement is transient viewing state like the widths.
