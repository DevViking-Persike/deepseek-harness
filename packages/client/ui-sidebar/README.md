...
The foot's action row (`sidebar.footer.action`) opens with this package's own entry: a side-switch button that calls `ctx.layout.toggleSidebarSide()`, moving the whole column between the left and right edges of the frame. The control ships with the column it moves, while [ui-layout](../ui-layout/README.md) owns the placement state.
