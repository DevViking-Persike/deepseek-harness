...
同一条推送还经 `ctx.conversation.imageSupport` 传递图片能力，由当前选择在 catalog 中的条目推导：声明的模态列表包含 `image` 则接受，不含则拒绝，其余情形——尚无选择、模型不在建议分组中、条目未声明任何模态——都表示无法判断。这里的三态与读取 `routable` 的方式一致，理由相同：缺失不是否定回答，把它当作否定会拒绝模型本可接受的附件。
