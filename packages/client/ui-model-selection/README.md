...
The same push carries image capability through `ctx.conversation.imageSupport`, derived from the current selection's catalog entry: a declared modality list containing `image` accepts, one without it refuses, and everything else — no selection yet, a model absent from the advisory groups, an entry declaring no modalities — cannot say. The tri-state matches how `routable` is read here, for the same reason: absence is not a negative answer, and treating it as one would refuse attachments a model accepts.
