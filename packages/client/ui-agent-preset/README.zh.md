...
agent preset 的各个表层：General 设置中的一行，用于选择新建会话据以组装的 [preset](../../preset/agent-presets/README.md)；composer 工具行中的一枚 chip，用于选择**空白会话**的 preset；会话标题旁的一个原地 preset 选择器；以及一个设置页分区，用于管理名单——复制、删除、默认值，以及通往 preset 自身文件的入口。
...
第二个表层，`conversation.input.left` 中的一个条目，位于访问模式与 plan 装饰旁边。用户决定发什么的时候人就在这里，而这一刻组装恰恰仍然可选；与一掠而过的界面不同，composer 在整个空白状态期间始终摆在用户面前。

它只在该状态成立时渲染。`session.blank` 经该槽位的 `InputZone` owner 份额抵达，因此 chip 读到的正是宿主推导出的同一个位；首条被接受的提示会让它卸载，而不是留下一个此后整个会话都处于禁用状态的控件。显示职责自此交给标题旁的标签，二者互斥：会话为空白期间标签自行隐去，因此同一份事实绝不会既有实时选择器又有静态名称。

chip 以部署默认值打开，其选择是**暂存**的——选择可以先于它要应用到的会话发生（设置页的创建入口先暂存再开启会话，而冷启动时根本还没有会话）。暂存值会在某个会话成为当前会话且仍为空白时抵达该会话；这既覆盖工作区连接新建的会话，也覆盖它复用的那个空白会话，而搭 `sessions.create` 的便车会漏掉后者。暂存值一经使用即被清空，因此下一个新会话重新以默认值打开——与新建会话界面上的工作区选择器完全一致。

已经开始的会话会被直接拒绝而非排队：宿主返回 `agent-preset-locked`，暂存值随之丢弃，而不是去等一个永远不会接受它的会话。

## 会话标题旁的选择器

第三个表层，位于会话标题旁：**本会话**所运行的 preset，作为一个原地重组同一会话的选择器。选择另一个 preset 会调用 `agentPreset.select`；会话、其历史与工作区保持不变，下一次请求只组装新组装的提示词与工具 schema。已完成的历史是持久证据——已完成的工具调用/结果对即使在新组装不再提供这些工具时仍是有效的提供商历史。宿主仅在轮次运行中拒绝（`agent-preset-locked`），选择器将该错误显示在 tooltip 中；替换失败会在宿主侧恢复原来的组装。它从会话自身的摘要同时读取 preset 与 `blank` 位——互斥的两半出自同一个来源——并在会话开始之前不渲染任何内容，因为在那之前显示由 composer chip 掌管。显示名称在 General 行所读的同一份名单上解析。转发的 owner 事件 `agent-preset/selected` 会在每个标签页中把已经提交的切换折进这份共享摘要；发起方标签页可能已经采用 RPC 回执，而合并是幂等的。
...
复制旁边是对话式入口：名单携带自指的 `cordis` preset 时，一张虚线添加卡（模型页的同款样式）会暂存它并开启新会话——分区经外壳的 owner-prop `close` 关闭设置面板，composer chip 自己的应用器负责组装工作区流程产出的空白会话。seat 会防止晚到的名单加载回退显示：暂存选择优先，其次是当前会话已携带的组装，最后才是部署默认值。
...
名单行携带 `broken`（宿主的形状检查发现组装缺失或不可加载）时渲染为标记卡片：红色边框、「加载失败」徽记（discovery 观察到的事实，而非断言文件已损坏——常见起因是用户刚编辑或删除了组装文件）、原样展示的原因、卡片主体禁用——它不能成为默认——复制也禁用，因为损坏 preset 的副本只是又一个损坏的 preset。损坏的自定义行保留位置与删除动作：文件正是修复它的地方，而删除正是清掉幽灵目录（组装文件被手动删除、目录仍占着 id）的方式；损坏的内置行连查看器也不提供——没有可读的组装可展示。两个选择器（通用设置行与 composer chip）则完全不列出损坏的 preset：它们选的是下一个会话的组装，列出无法组装的选项只会把失败推迟到会话启动。
