...
import type { ImageSupport } from './input/image-support.ts'
...
export interface IConversation {
  ...
  /**
   * The per-session image-capability registry: how a plugin the composer
   * cannot import says whether the current model accepts image input.
   */
...
export class ConversationController extends Service implements IConversation {
  ...
  readonly blocks: ComposerBlocks
  /** The per-session image-capability registry. */
  readonly imageSupport: ImageSupport
   ...
   */
  constructor(ctx: Context, config: { input: SessionInputResolver; blocks: ComposerBlocks; imageSupport: ImageSupport }) {
    super(ctx, 'conversation')
    this.input = config.input
    this.blocks = config.blocks
    this.imageSupport = config.imageSupport
    ctx.effect(() => () => {
      this.disposed = true
