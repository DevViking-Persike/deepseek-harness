...
function scriptedApi(overrides: {
  ...
  return {
    ...
    editor: {
      languageServers: r => ok(r, { servers: [] }),
      listDir: r => ok(r, { path: '/w', root: '/w', entries: [] }),
    ...
    docker: {
      ...
      installEngine: r => ok(r, { status: { running: false, startable: true, installable: false }, output: '' }),
      listContainers: r => ok(r, { containers: [] }),
      control: r => ok(r, { container: { id: r.payload.container, name: r.payload.container, image: 'nginx', state: 'running', status: 'Up', ports: [], createdAt: '0' } }),
      listImages: r => ok(r, { images: [] }),
      logs: r => ok(r, { container: r.payload.container, content: '', truncated: false }),
      browseCompose: r => ok(r, { path: r.payload.path ?? '/h', home: '/h', crumbs: [], entries: [], truncated: false }),
      composeUp: r => ok(r, { project: r.payload.project ?? 'p', output: '', containers: [] }),
      composeDown: r => ok(r, { project: r.payload.project ?? 'p', output: '', containers: [] }),
    },
    agentPresets: {
