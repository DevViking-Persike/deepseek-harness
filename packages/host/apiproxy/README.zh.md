...
`agentPreset.list` 领域向浏览器暴露部署的 preset 名单，使其在开启会话时能够提供选择；每一行携带它的 `trust`（`user` preset 的权限恰好等于它所引用的插件）、它是否为当前默认值，以及——当该 preset 无法组装会话时——一条 `broken` 原因：损坏的目录仍占着它的 id，界面必须能展示并删除它，而不是把它端出来然后在会话启动时失败。未组装任何 preset 的部署返回空名单而非错误，因为共用宿主组装本身就是一种有效部署。`agentPreset.select` 用另一个 preset 原地重组某个会话的 agent——会话、其历史与工作区保持不变。已开始的会话在其轮次落定后即可切换：已完成的历史是持久证据，已完成的工具调用/结果对即使在新组装不再提供这些工具时仍是有效的提供商历史，下一次请求只组装新组装的提示词与工具 schema。轮次运行中返回 `agent-preset-locked`：此时的替换会使某一步的请求与其工具执行之间在途的工作搁浅。替换失败会恢复原来的组装。
...
`docker.*` 领域是对宿主 `ctx.docker` 能力接缝的只读观测：`docker.listContainers`、`docker.listImages` 与 `docker.logs`。生命周期操作被刻意排除在外——启停容器是 agent 经由 [`@deepseek-ai/dsh-tool-docker`](../../docker/tool-docker/README.md) 完成的工作，在那里它会作为工具调用落账，而一个不落账的按钮会在没有会话记录的情况下改变机器状态。接缝抛出的全部五种提供者选择失败（没有可用提供者、多个可用提供者而未钉死其一、配置的 id 未注册或够不到引擎），连同根本未组装该接缝的组合，统一收敛为一个 `docker-unavailable` 码：从浏览器一侧看，它们是同一种空状态而不是五种错误；其余接缝或引擎失败折叠为 `internal` 并携带底层消息。`docker.logs` 以网关的 `dockerLogMaxChars`（默认 40000）为界限，并保留**最新**的字符，因为日志的尾部才解释刚刚发生的失败；当宿主或提供者丢弃了更早的条目时，响应报告 `truncated`。
